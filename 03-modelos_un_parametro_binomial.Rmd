...
\theta &\sim p(\theta)
\end{align}
Así, la **distribución muestral (distribución condicional conjunta)** de $\boldsymbol{y} = (y_1,\ldots,y_n)$ dado $\theta$ es
$$
...
**Diaconis, P. & Ylvisaker, D. (1985). Quantifying prior opinion (with discussions), Bayesian Statist. 2.**
...
**(Propiedad.)** Los resultados Bayesianos son **aproximadamente iguales a los frecuentistas** si: 1. la distribución previa es difusa (no informativa) y 2. el tamaño de la muestra es "grande".
...
También, la **distribución predictiva posterior** de una observación futura $y*$ es tal que
$$
\textsf{Pr}(y^*=1\mid s) = \frac{a+s}{a+b+n}\,.
$$
La distribución predictiva posterioir **no depende** de cantidades desconocidas. La distribución predictiva depende de los datos. Por tal motivo, $y^*$ no es independiente de $s$.
...
Datos de las víctimas de violencia sexual suministrados por el **Observatorio de Memoria y Conlficto** y el **Centro Nacional de Memoria Histórica** disponibles en https://micrositios.centrodememoriahistorica.gov.co/observatorio/portal-de-datos/base-de-datos/.
...
legend("topright", legend = c("Previa", "Posterior", "IC 95%", "Media"), col = c(1, 4, 2, 3), 
       lty = 1, lwd = 2, bty = "n")
...
for (agno in c(2000:2021)) {
      ...
      # media posterior, intervalos de credibilidad al 95% y 99%, cv
...
```{r}
plot(x = 1:nrow(out), y = out[,3], ylim = c(0.25,1), pch = 19, xaxt = "n", xlab = "Año", ylab = expression(theta))
lines(x = 1:nrow(out), y = out[,3], type = "l", col = 4)
segments(x0 = 1:nrow(out), y0 = out[,5], x1 = 1:nrow(out), y1 = out[,6], lwd = 3)
segments(x0 = 1:nrow(out), y0 = out[,7], x1 = 1:nrow(out), y1 = out[,8], lwd = 1)
...
Simular $N = 1000000$ veces $s\sim\textsf{Bin}(n,\theta)$ para $\theta\in\{0.5,0.7,0.9,0.99\}$ y $n\in\{5,30,100\}$, para calcular la cobertura frecuentista (calibración) del intervalo de credibilidad y el intervalo de confianza para $\theta$ con base la muestra aleatoria de tamaño $n$.
...
colnames(out) <- c("theta","n","Conf. Bayes.","Conf. Frec.","Prop. Bayes. > 0","Prop. Frec. > 0")
knitr::kable(x = out, digits = 2, align = "c")
```

```{r, eval=T, echo=F}
load("03-modelos_un_parametro_binomial_simulacion.RData")
knitr::kable(x = out, digits = 2, align = "c")
```
