...
Sea $\theta$ un parámetro de interés y $\boldsymbol{y} = (y_1,\dots,y_n)$ un conjunto de observaciones. Supongamos que **es posible obtener una muestra aleatoria** de $B$ valores de $\theta$ asociados con la **distribución posterior** $p(\theta\mid \boldsymbol{y})$, esto es,
$$
...
**(Ley débil de los grandes números).** Sea $X_1,\ldots,X_n$ una secuencia de variables aleatorias independientes e identicamente distribuidas con media $\mu$ y varianza finita $\sigma^2$. Entonces, el promedio muestral $\bar{X} = \frac{1}{n}\sum_{i=1}^n X_i$ converge en probabilidad a $\mu$ cuando $n\rightarrow\infty$, i.e., para todo $\epsilon > 0$ se tiene que
$$
...
$$
- **Percentíl** $\alpha$ de $\theta^{(1)},\ldots,\theta^{(B)}\longrightarrow \theta_{\alpha}$
- **Distribución empírica** de $\theta^{(1)},\ldots,\theta^{(B)} \longrightarrow p(\theta\mid \boldsymbol{y})$.
...
hist(x = gamma_mc, freq = F, col = "gray95", border = "gray95", xlim = mean(gamma_mc) + 4*c(-1,1)*sd(gamma_mc), xlab = expression(gamma), ylab = expression(paste("p(",gamma," | y)", sep="")), main = "")
...
round(tab, 3)
# probabilidades de la distribucion predictiva mejeres sin pregrado o menos
...
# grafico
par(mar = c(3,3,1.4,1.4), mgp = c(1.75,.75,0))
hist(x = t_mc, freq = F, col = "gray95", border = "gray95", xlab = "t", ylab = "p(t | y)", main = "")
...
# grafico
par(mfrow = c(1,2))
# simple diseño de Fruchterman y Reingold
set.seed(1234)
...
# decorado diseño de Fruchterman y Reingold
data(lazega, package = "sand")
v.colors <- c("red","dodgerblue","goldenrod")[V(lazega)$Office] # color: ubicacion de la oficina
v.shapes <- c("circle","square")[V(lazega)$Practice]  # forma del vertice: tipo de practica 
v.size   <- 3.5*sqrt(V(lazega)$Years)  # tamaño del vertice: proporcional a años de servicio
v.label  <- V(lazega)$Seniority  # etiquetas
set.seed(1234)
plot(lazega, layout = layout_with_fr, vertex.color = v.colors, vertex.shape = v.shapes, vertex.size = v.size, vertex.label = v.label, edge.color = "gray50", vertex.frame.color = "black", vertex.label.color = "black")
box()
title(main = "Trabajo colaborativo")
```
...
# inferencia
tab <- rbind(c(mean(theta_mc), quantile(x = theta_mc, probs = c(.025,.975))), c(ap/(ap + bp), qbeta(c(.025,.975), shape1 = ap, shape2 = bp)))
colnames(tab) <- c("Estimación", "Q2.5%", "Q97.5%")
rownames(tab) <- c("Aproximada","Exacta")
round(tab, 4)
```

```{r}
# estadisticos de prueba
t0 <- as.matrix(c(mean(igraph::degree(lazega)), sd(igraph::degree(lazega)), igraph::edge_density(lazega), igraph::transitivity(lazega)))
colnames(t0) <- c("Lazega")
rownames(t0) <- c("Gr. Prom.", "Gr. Desv. Est.", "Densidad","Transitividad")
round(t0, 3)
```
...
for (i in 1:B) {
  ...
  g <- igraph::graph_from_adjacency_matrix(adjmatrix = Y + t(Y), mode = "undirected")
  # estadisticos
  t_mc <- rbind(t_mc, c(mean(igraph::degree(g)), sd(igraph::degree(g)), igraph::edge_density(g), igraph::transitivity(g)))
}
dim(t_mc)
head(t_mc)
```
...
for (k in 1:4) {
  # ppp (valor p predictivo posterior)
  ppp <- round(mean(t_mc > t_obs), 3)
  t_obs <- t0[k]
  hist(x = t_mc[,k], freq = F, col = "gray95", border = "gray95", xlab = "t", ylab = "p(t | y)", main = paste0(nombres[k], ", t = ", round(t0[k], 3), ", ppp = ", ppp))
  lines(density(t_mc[,k]), col = 4, lwd = 2)
  abline(v = t_obs, col = 1, lwd = 2, lty = 1)
  abline(v = quantile(x = t_mc[,k], probs = c(0.025, 0.975)), lty = 2, lwd = 2, col = 2)
}
```
