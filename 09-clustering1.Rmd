...
abline(v = 3, lty = 4, col = 2)
```

## Inferencia: $H=3$

```{r}
# numero de grupos optimo
H <- 3
```
...
# resumen posterior
resumen_pos <- function(x) round(c(mean(x), sd(x), quantile(x = x, probs = c(0.025,0.975))), 3)
tab <- rbind(resumen_pos(muestras3$THETA[,1]),
             resumen_pos(muestras3$THETA[,2]),
             resumen_pos(muestras3$THETA[,3]),
             resumen_pos(muestras3$SIG2),
             resumen_pos(muestras3$OMEGA[,1]),
             resumen_pos(muestras3$OMEGA[,2]),
             resumen_pos(muestras3$OMEGA[,3]))
colnames(tab) <- c("Media","SD","Q2.5%","Q97.5%")
rownames(tab) <- c(paste0("theta_",1:H),"sig2",paste0("omega_",1:H))
knitr::kable(x = tab, digits = 3, align = "c")
```
...
for(i in 1:M) {
  y0[i] <- f_true(x0[i])
  for (b in 1:B)
    FE[b,i] <- sum(muestras3$OMEGA[b,]*dnorm(x = x0[i], mean = muestras3$THETA[b,], sd = sqrt(muestras3$SIG2[b])))
}
f_hat <- colMeans(FE)
f_inf <- apply(X = FE, MARGIN = 2, FUN = quantile, probs = 0.025)
f_sup <- apply(X = FE, MARGIN = 2, FUN = quantile, probs = 0.975)
